...
/* CARREGA OS SIMBOLOS DINAMICOS DE UM BINÁRIO */
...
/* CARREGA OS SIMBOLOS ESTÁTICOS DE UM BINÁRIO */
/* NA LIBBFD OS SIMBOLOS SÃO REPRESENTADOS PELA ESTRUTURA bfd_symbol (NOMEADOS COMO asymbol)*/
/* UMA TABELA DE SIMBOLOS É REPRESENTADA COMO UM ARRAY DE PONTEIROS (asymbol**) */
static int load_symbols_bfd(bfd *bfd_h, Binary *bin) {
  ...
  n = bfd_get_symtab_upper_bound(bfd_h); // retorna o tamanho em bytes da tabela de simbolos
  ...
  if (n < 0) {
    ...
    bfd_symtab = (asymbol**)malloc(n); // aloca o "array" de ponteiros que representa a tabela de simbolos
    if (!bfd_symtab) {
      fprintf(stderr, "out of memory\n");
      goto fail;
    }

    nsyms = bfd_canonicalize_symtab(bfd_h, bfd_symtab); // função da libbfd que preenche nosso array de ponteiros com referências aos simbolos
    if (nsyms < 0) {
      fprintf(stderr, "failed to read symtab (%s)\n",
      bfd_errmsg(bfd_get_error()));
      goto fail;
    ...
    for (i = 0; i < nsyms; i++) {
      if(bfd_symtab[i]->flags && BSF_FUNCTION) {
        bin->symbols.push_back(Symbol()); // adiciona um novo simbolo
        sym = &bin->symbols.back(); // captura a referência ao ultimo elemento do vetor 
        sym->type = Symbol::SYM_TYPE_FUNC;
        sym->name = std::string(bfd_symtab[i]->name);
        sym->addr = bfd_asymbol_value(bfd_symtab[i]); // função da libbfd que captura o endereço do simbolo
      }
    }
  }
...
/* Retorna um ponteiro para o identificador BFD */
static bfd* open_bfd(std::string &fname) {
  static int bfd_inited = 0;
  bfd *bfd_h;

  if (!bfd_inited) {
    /* Incializa o estado interno do libbfd */
    bfd_init();
    bfd_inited = 1;
  }
  
  /* Abre o binário */
  /* retorna um ponteiro para um arquivo tipo bfd ou NULL em caso de erro */
  ...
  /* Verifica se o tipo do arquivo aberto é um bfd_object (executável, objeto realocável ou shared lib) */
  ...
  /* Verifica se o binário é de um tipo conhecido (ELF/PE/ETC) */
